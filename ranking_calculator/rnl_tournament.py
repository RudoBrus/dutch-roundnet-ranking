...
class RNLTournament:
    def __init__(
        ...
        self.tournament_data["player_id"] = (
            self.tournament_data["name"].str.lower().map(reverse_playerbook)
        )

        if self.tournament_data["player_id"].isna().any():
            missing_players = self.tournament_data[
                self.tournament_data["player_id"].isna()
            ]["name"].tolist()
            raise ValueError(
                f"Players not found in playerbook: {', '.join(missing_players)}"
            )
    ...
    def map_tournament_points(self):
        ...
        self.tournament_data["blank_points"] = self.tournament_data["rank"].map(
            blank_points
        )
    ...
    def calculate_tournament_points(self):
        ...
        self.tournament_data["tournament_points"] = (
            self.tournament_data["blank_points"] * self.level_multiplier
        )
